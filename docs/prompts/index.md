...
这个提示词文件很杂乱，什么东西都有。不要考虑做精细化管理，想写什么就写。
...
## 029 <!-- TODO: 引申出一系列子问题，正在逐步探究 --> AI 协作开发推荐的 UI 视觉能力调整的工具
...
1. 缺少审美与设计约束：AI 不知道应该设计成什么风格。
2. 缺少可迭代的视觉原型载体：只有代码，没有真正的设计过程。
3. 缺少可定位、可测量、可验证的反馈闭环：你说“这里不好看”，AI 不知道“这里”是哪一个 DOM、应改多少、改完如何验收。

- Open Design / Figma / Pencil 负责设计
- 21st Toolbar 或 Jam 负责指向性反馈
- Playwright + Chrome DevTools 负责验证。

## 030 <!-- 已询问 内容很繁琐，很怀疑单纯的Open Design本身就可以完成提取了，具体提取方式自己没必要深入学习，过于细致了 --> 如何提取设计元素？

我有一个初稿就很美观的，视觉效果很统一清晰的 vue+typescript+vite 大屏前端项目。可是在持续的维护迭代时，AI 的设计能力逐步出现跑偏和降级的情况。因为我的项目缺少了严格的视觉规范约束文档。

1. 我应该怎么在项目根目录内，建立一个严格的 design.md UI 设计能力的约束文档？我应该用 design.md 来约束什么行为？
...
## 031 <!-- 已询问 AI的回答过于复杂，过度工程化了。我不相信用大量的截图来作为视觉参考基线的方案。 --> 如何建立一个合适的本地视觉验证的工件目录？
...
## 032 <!-- 已询问 同样的，AI给的方案几乎就是一整套skills了，太长了。还设计了一套定位vue组件的标记 太复杂了 --> 如何在生成项目原型与 UI 设计时，确保生成的格式是 AI 可读的验收格式？怎么设置这样的 AI agent 可阅读的高效迭代格式？
...
验证视觉合同 Schema
    ↓
读取 approved reference
    ↓
加载固定 Fixture
    ↓
打开固定 Viewport
    ↓
获取 ARIA Snapshot + before.png
    ↓
运行硬断言
    ↓
将失败定位到 data-ui 区域
